...
__global__ void sumMatrixOnDevice1D(float *A, float *B, float *C, const int nx, int ny)
{
    unsigned int ix = threadIdx.x + blockIdx.x * blockDim.x;
    if (ix < nx) {
    ...
    //printf("threadidx: %d\tblockIdx: (%d, %d)\tblockDim(%d, %d)\n", 
    //        threadIdx.x, blockIdx.x, blockIdx.y, blockDim.x, blockDim.y);
}
...
int main(void)
{
    ...
    dim3 grid((block.x + nx - 1) / block.x, 1);
    printf("block: (%d, %d), grid: (%d, %d)\n", block.x, block.y, grid.x, grid.y);

    start = getCpuSec();
    sumMatrixOnDevice1D<<<grid, block>>>(d_A, d_B, d_C, nx, ny);
    printf("GPU: %lf\n", getCpuSec() - start);
